...
const Footer = () => {
  return (
    <div className='w-full min-h-full p-16 max-md:px-6 flex gap-5 justify-between'>
        <div className='w-1/2  max-md:w-[30%] flex flex-col justify-between'>
          <div>
            <h1 className='font-["Grot"] uppercase font-semibold leading-none text-[10vw] -mb-10 max-md:-mb-2 max-md:text-[8vw] '>eye-</h1>
            <h1 className='font-["Grot"] uppercase font-semibold leading-none text-[10vw] -mb-10 max-md:-mb-2 max-md:text-[8vw] '>opening-</h1>
          </div>
          <div>
        ...
        <div className='w-1/2  max-md:w-[70%] '>
            <h1 className='font-["Grot"] uppercase font-semibold leading-none text-[9vw] -mb-10 '>presentations
            </h1>
            <div className='mt-16'>
              <p className='mb-5'>S:</p>
              {["Instagram","Behance","Facebook","Linkedin"].map((items,index)=>{return <a key={index} className='block'>{items}</a>})}
            </div>

            <div className='flex justify-between'>
                <div className='mt-5'>
                  <p className='mb-4'>L:</p>
                  {["202-1965 W 4th Ave","BehVancouver, Canadaance","30 Chukarina St","Lviv, Ukraine"].map((items,index)=>{return <a key={index} className={`block ${index===2&&"mt-5"}`}>{items}</a>})}
                </div>
                <div className='mt-5'>
                  <p className='mb-4'>M:</p>
                  {["Home","Services","Our work","About us","Insights","Contact us"].map((items,index)=>{return <a key={index} className={`block `}>{items}</a>})}
                </div>
            </div>

            <div className='mt-6'>
              <p className='mb-5'>S:</p>
              <a href="#">ansarwebdeveloper@gmail.com
              </a>
            </div>

            <div className='mt-12 flex justify-between text-zinc-500 font-semibold'>
              <p>© ochi design 2025</p>
              <p>Website by <b className='text-[#CDEA68]'>Ansar</b></p>
            </div>
